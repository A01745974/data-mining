...
Escribe la fecha actual (**NOTE-01**): 31 de Mayo del 2021
...
```{r}
NOMBRE_COMPLETO = "Luz del Carmen Duarte"
DIGITOS_MATRICULA = 1745974
```
...
```{r}
try(
  plot(smalltx, main = paste0("Elaborado por: Luz del Carmen Duarte ", " (",1745974, ")" ))  
)
...
```{r}
try(
  itemFrequencyPlot(smalltx, topN=10, main = paste0("Elaborado por:Luz del Carmen Duarte ", " (", 1745974, ")" ))
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):En esa primera regla no existe una relación del producto de pañales con otro artículo
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque las métricas de support y confidence excluyen a la regla
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Representa cada una de las sesiones del dataset, su propósito es distinguir el número de sesiones que los usuarios tienen en Spotify para poder hacer recomendaciones
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Representa cada una de las canciones, distinguir cuáles son las canciones que escuchan ciertos usuarios y con base en sus preferencias hacer recomendaciones
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que representan el comportamiento que el usuario en Spotify tiene al momento de escuchar música. Se pueden hacer asociaciones si se entiende la relación de las canciones con los gustos, a través del número de canciones saltadas, pausas, etc
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce y frecuencia 874 
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética?20 (**NOTE-28**): Los usarios tienen en promedio 14.58 sesiones en promedio
...
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que se pierda precisión en el análisis, pues se tendrá un mayor número de reglas de asociación que puede que no sean tan certeras
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por: Luz del Carmen Duarte ", " (",1745974, ")" ))
)
